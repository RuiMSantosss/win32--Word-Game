...
PLAYER* verifyScores(APP_CONTEXT* appContext, FLOAT oldScore, PLAYER* playerToAttend) {
	FLOAT highestScore = 0;
	DWORD iPreviousHighestScore = 0;
	DWORD iPostHighestScore = 0;
	for (DWORD i = 0; i < appContext->numActivePlayers; i++) {
		if (_tcscmp(appContext->activePlayers[i].name, playerToAttend->name) == 0) {
...
void readOrCreateRegistryValues(APP_CONTEXT* appContext) {
	...
	if (maxLetters > MAX_MAXLETTERS) {
		maxLetters = MAX_MAXLETTERS;
		RegSetValueEx(hKey, KEY_MAXLETTERS, 0, REG_DWORD, (LPBYTE)&maxLetters, sizeof(DWORD));
	}
	size = sizeof(DWORD);
...
DWORD WINAPI playerAttend(LPVOID dataArg) {
	...
	size_t wordLength;
	TCHAR copyName[PLAYER_NAME_TAM];
	_tcscpy_s(copyName, PLAYER_NAME_TAM, playerToAttend->name);
	...
	do {
		...
		case WORDGUESS:
			...
			if (!verifyWordIsCorrect(letters, messagePlayer.message)) {
				...
				EnterCriticalSection(&criticalSection);
				(playerToAttend->score) -= ((FLOAT)wordLength / 2);
				if ((playerToAttend->score) <= 0) {
					(playerToAttend->score) = 0;
				}
				LeaveCriticalSection(&criticalSection);
			}
				...
				EnterCriticalSection(&criticalSection);
				(playerToAttend->score) += wordLength;
				for (i = 0; i < pThreadAttend->pAppContext->numActivePlayers; i++) {
					if (_tcscmp(pThreadAttend->pAppContext->activePlayers[i].name, playerToAttend->name) == 0) {
						continue;
					}
			...
			EnterCriticalSection(&criticalSection);
			PLAYER* newLeader = verifyScores(pThreadAttend->pAppContext, oldScore, playerToAttend);
			LeaveCriticalSection(&criticalSection);
			if (newLeader != NULL) {
